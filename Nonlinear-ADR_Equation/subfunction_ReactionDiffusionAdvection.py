...
def pde_loss_ReactionDiffusionAdvection(x_max, x_min, t_max, t_min, device, u_net, D_net, N_PDE, G_net, V_net):
...
# 1) plot the learnt u, D, V, G
def learnt_u_D_G_plot(Nt, x_train, u_true, test_u_pred, D, G_true, U_grid, test_D_pred_np, test_G_pred_np, u_min, u_max, save_dir, savename, V_true, test_V_pred_np):
    ...
    G_true, D, V_true = process_V(G_true, len(U_grid)), process_D(D, len(U_grid)), process_V(V_true, len(U_grid))
    ...
    # 4). learnt V
    axes[3].plot(U_grid, V_true, color='black', linestyle='--', label=r'True $V(u)$', linewidth=3) # true V
    axes[3].plot(U_grid, test_V_pred_np, linewidth=3, alpha=0.5, color='blue') # learnt V
    axes[3].set_title(rf'Learnt $V_{{\zeta}}(u)$')
    axes[3].set_xlabel(r'$u$')
    axes[3].set_ylabel(r'$V_{\zeta}(u)$')
    axes[3].set_ylim(-0.1, 1.1)
    axes[3].set_xlim(u_min, u_max)
    axes[3].legend()
...
def model_performance_eval(save_dir, u_net, D_net, test_loader, device, t_train, x_train, U_grid_tensor, Nt, u_true, D, U_grid, u_min, u_max, X_grid, T_grid,\
                BCloss_S, pdeloss_S, val_BCloss_S, val_pdeloss_S, Numrep, store_loss_step, val_u_bio_S, val_D_bio_S, u_bio_S, D_bio_S, uloss_S, val_uloss_S, Num_epochs, \
                    G_true, G_net, bio_constraint_bool, V_true, V_net):
    ...
    for i in range(Numrep):
        ...
        V_net = torch.load(f"{save_dir_i}V_net_best_Iter{i}.pth", map_location=device, weights_only=False)
        # evaluate model
        u_net.eval()
        D_net.eval()
        G_net.eval()
        ...
        with torch.no_grad():
            ...
            learnt_u_D_G_plot(Nt, x_train, u_true, u_pred_np_full, D, G_true, U_grid, D_pred_full, G_pred_full, u_min, u_max, save_dir_i, savename, V_true, V_pred_full)
...
def TrainBINN(save_dir, device, D, t_end, batch_size, Numrep, Num_epochs, u_num_layers, u_hidden_units, u_activation_fn, \
    ...
    V_predS = [] # evaluate the model to obtain V values at the u grid points 'U_grid_tensor' for each repetition
    ...
    for i in range(Numrep):
        ...
        optimizer = torch.optim.Adam(
            list(u_net.parameters()) + list(D_net.parameters()) + list(G_net.parameters()) + list(V_net.parameters()),
        ...
        for epoch in range(Num_epochs):
            ...
            for _, (x_batch, u_batch) in enumerate(train_loader):
                ...
                pde_loss = pde_loss_ReactionDiffusionAdvection(x_max, x_min, t_end, t_min, device, u_net, D_net, N_PDE, G_net, V_net)
                ...
                if bio_constraint_bool:
                    u_bio_loss, D_bio_loss = biology_loss(x_max, x_min, t_min, t_end, device, u_net, D_net, N_bio, G_net)
            ...
            for x_batch_val, u_batch_val in val_loader:
                ...
                # 2) PDE constraint loss (based on sampled 'N_PDE' virtual data):
                pde_loss_ = pde_loss_ReactionDiffusionAdvection(x_max, x_min, t_end, t_min, device, u_net, D_net, N_PDE, G_net, V_net)
                ...
                u_bio_loss_, D_bio_loss_ = biology_loss(x_max, x_min, t_min, t_end, device, u_net, D_net, N_bio, G_net)
