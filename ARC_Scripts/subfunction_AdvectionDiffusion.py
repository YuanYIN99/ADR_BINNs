        ...
        if hasattr(self, "network"):
            for m in self.network:
                if isinstance(m, nn.Linear):
                    ...
                    if m.bias is not None:
                        init.zeros_(m.bias)
                    ## --- ADDED: Manually increase weight magnitude ---
                    #m.weight.data.mul_(5.0)
        else:
            init.xavier_normal_(self.hidden_layer.weight)
            if self.hidden_layer.bias is not None:
                init.zeros_(self.hidden_layer.bias)
...
def compute_J_dx(u, d, v, x_input):
...
# 2) PDE loss for for advection-diffusion equation u_t = (D(u)u_x0-V(u)*u)_x
def pde_loss_AdvectionDiffusion(x_max, x_min, t_max, t_min, device, u_net, D_net, N_PDE, V_net):
    ...
    V_pred_rand = V_net(u_pred_rand)

    # PDE constraint loss: 
    _, du_dt = compute_du_dxdt(u_pred_rand, inputs_rand)
    J_x = compute_J_dx(u_pred_rand, D_pred_rand, V_pred_rand, inputs_rand)
...
def learnt_u_D_V_plot(Nt, x_train, u_true, test_u_pred, D, V_true, U_grid, test_D_pred_np, test_V_pred_np, u_min, u_max, save_dir, savename):
    ...
    V_true, D = process_V(V_true, len(U_grid)), process_D(D, len(U_grid)) 
    ...
    # 3). learnt V
    axes[2].plot(U_grid, V_true, color='black', linestyle='--', label=r'True $V(u)$', linewidth=3) # true V
    axes[2].plot(U_grid, test_V_pred_np, linewidth=3, alpha=0.5, color='blue') # learnt V
    axes[2].set_title(rf'Learnt $V_{{\zeta}}(u)$')
    axes[2].set_xlabel(r'$u$')
    axes[2].set_ylabel(r'$V_{\zeta}(u)$')
...
# 2) relative error of the learnt u, D, V
def u_D_V_relerr(test_u_pred, u_true, X_grid, T_grid, t_train, x_train, test_D_pred_np, D, U_grid, save_dir, savename, V_true, test_V_pred_np):
    ...
    # 3). relative error of V
    V_relerrS = (test_V_pred_np - V_true) / V_true
    ax3 = fig2.add_subplot(gs[0, 2])
    ax3.plot(U_grid, V_relerrS, linewidth=3, color='blue')
    ax3.set_title(rf'Relative error: $(V_{{\zeta}} - V)/V$')
    ax3.set_xlabel(r'$u$')
    ax3.set_ylabel(r'$(V_{\zeta} - V)/V$')
...
def model_performance_eval(save_dir, u_net, D_net, test_loader, device, t_train, x_train, U_grid_tensor, Nt, u_true, D, U_grid, u_min, u_max, X_grid, T_grid,\
                BCloss_S, pdeloss_S, val_BCloss_S, val_pdeloss_S, Numrep, store_loss_step, val_u_bio_S, val_D_bio_S, u_bio_S, D_bio_S, uloss_S, val_uloss_S, Num_epochs, \
                    V_true, V_net, bio_constraint_bool):
    ...
    l2re_uS_full, l2re_DS_full, l2re_VS_full = np.zeros(Numrep), np.zeros(Numrep), np.zeros(Numrep) # relative L2 error
    l1re_uS_full, l1re_DS_full, l1re_VS_full = np.zeros(Numrep), np.zeros(Numrep), np.zeros(Numrep) # relative L1 error
    mse_uS_full, mse_DS_full, mse_VS_full = np.zeros(Numrep), np.zeros(Numrep), np.zeros(Numrep)    # mean squared error
    merr_uS_full, merr_DS_full, merr_VS_full = np.zeros(Numrep), np.zeros(Numrep), np.zeros(Numrep) # maximum absolute error
    # 2) on test data:
    num_batches = len(test_loader) # to average the metrics over batch numbers
    l2re_uS, l2re_DS, l2re_VS = np.zeros(Numrep), np.zeros(Numrep), np.zeros(Numrep) # relative L2 error
    l1re_uS, l1re_DS, l1re_VS = np.zeros(Numrep), np.zeros(Numrep), np.zeros(Numrep) # relative L1 error
    mse_uS, mse_DS, mse_VS = np.zeros(Numrep), np.zeros(Numrep), np.zeros(Numrep)    # mean squared error
    merr_uS, merr_DS, merr_VS = np.zeros(Numrep), np.zeros(Numrep), np.zeros(Numrep) # maximum absolute error
    ...
    for i in range(Numrep):
        ...
        V_net = torch.load(f"{save_dir_i}V_net_best_Iter{i}.pth", map_location=device)
        # evaluate model
        u_net.eval()
        D_net.eval()
        V_net.eval()
        ...
        with torch.no_grad():
            ...
            V_pred_full = V_net(U_grid_tensor.unsqueeze(1)).cpu().numpy().flatten()
            l2re_VS_full[i] = np.sqrt(np.sum((V_pred_full - V_true)**2)) / np.sqrt(np.sum(V_true**2))
            l1re_VS_full[i] = np.sum(np.abs(V_pred_full - V_true)) / np.sum(np.abs(V_true))
            mse_VS_full[i] = np.mean((V_pred_full - V_true)**2)
            merr_VS_full[i] = np.max(np.abs(V_pred_full - V_true))

            np.save(f'{save_dir_i}fullgrid_u_pred.npy', u_pred_np_full)
            np.save(f'{save_dir_i}fullgrid_D_pred.npy', D_pred_full)
            np.save(f'{save_dir_i}fullgrid_V_pred.npy', V_pred_full)

            # plot the learnt u and D and V
            savename = 'fullgrid_u_D_V_learnt.png'
            learnt_u_D_V_plot(Nt, x_train, u_true, u_pred_np_full, D, V_true, U_grid, D_pred_full, V_pred_full, u_min, u_max, save_dir_i, savename)
            # plot the relative error of the learnt u and D and V
            savename = 'fullgrid_u_D_V_learnt_relerr.png'
            u_D_V_relerr(u_pred_np_full, u_true, X_grid, T_grid, t_train, x_train, D_pred_full, D, U_grid, save_dir_i, savename, V_true, V_pred_full)
            
            # 2) Performance evaluation on the test dataset: 
            # initialize the metrics for each batch
            l2re_u, l2re_D, l2re_V = 0.0, 0.0, 0.0
            l1re_u, l1re_D, l1re_V = 0.0, 0.0, 0.0
            mse_u, mse_D, mse_V = 0.0, 0.0, 0.0
            merr_u, merr_D, merr_V = 0.0, 0.0, 0.0
            for x_batch, u_batch in test_loader:
                x_batch, u_batch = x_batch.to(device), u_batch.to(device)
                u_pred = u_net(x_batch)
                D_pred, V_pred = D_net(u_pred), V_net(u_pred)
                ...
                l2re_V += np.sqrt(np.sum((V_pred.cpu().detach().numpy() - V_true)**2) / np.sum(V_true**2))
                l1re_V += np.sum(np.abs(V_pred.cpu().detach().numpy() - V_true)) / np.sum(np.abs(V_true))
                mse_V += np.mean((V_pred.cpu().detach().numpy() - V_true)**2)
                merr_V += np.max(np.abs(V_pred.cpu().detach().numpy() - V_true))
            l2re_uS[i] = l2re_u / num_batches
            l2re_DS[i] = l2re_D / num_batches
            l2re_VS[i] = l2re_V / num_batches
            l1re_uS[i] = l1re_u / num_batches
            l1re_DS[i] = l1re_D / num_batches
            l1re_VS[i] = l1re_V / num_batches
            mse_uS[i] = mse_u / num_batches
            mse_DS[i] = mse_D / num_batches
            mse_VS[i] = mse_V / num_batches
            merr_uS[i] = merr_u / num_batches
            merr_DS[i] = merr_D / num_batches
            merr_VS[i] = merr_V / num_batches
    ...
    np.save(f'{save_dir}fullgrid_performance_V.npy', np.array([l2re_VS_full, l1re_VS_full, mse_VS_full, merr_VS_full]))
    # 2) test dataset:
    ...
    np.save(f'{save_dir}testdataset_performance_V.npy', np.array([l2re_V, l1re_V, mse_V, merr_V])) 
    pass
...
def TrainBINN(save_dir, device, D, t_end, batch_size, Numrep, Num_epochs, u_num_layers, u_hidden_units, u_activation_fn, \
    ...
    D_predS, V_predS = [], [] # evaluate the model to obtain D values at the u grid points 'U_grid_tensor' for each repetition
    ...
    for i in range(Numrep):
        ...
        V_net = VNet(V_num_layers, V_hidden_units, V_activation_fn).to(device)
        optimizer = torch.optim.Adam(
            list(u_net.parameters()) + list(D_net.parameters()) + list(V_net.parameters()),
        ...
        for epoch in range(Num_epochs):
            ...
            for _, (x_batch, u_batch) in enumerate(train_loader):
                ...
                pde_loss = pde_loss_AdvectionDiffusion(x_max, x_min, t_end, t_min, device, u_net, D_net, N_PDE, V_net)
            ...
            #     D_pred_full_, V_pred_full_ = D_net(u_pred_full_), V_net(u_pred_full_)
            #     _, LHS = compute_du_dxdt(u_pred_full_, grid_points_tensor)
            #     RHS = compute_J_dx(u_pred_full_, D_pred_full_, V_pred_full_, grid_points_tensor)
            ...
            for x_batch_val, u_batch_val in val_loader:
                ...
                # 2) PDE constraint loss (based on sampled 'N_PDE' virtual data):
                pde_loss_ = pde_loss_AdvectionDiffusion(x_max, x_min, t_end, t_min, device, u_net, D_net, N_PDE, V_net)
            ...
            if val_total_loss < totalloss_bestepoch_alliters[i]: # check if the validation loss has improved
                ...
                torch.save(V_net, f'{save_dir_i}V_net_best_Iter{i}.pth')
        ...
        with torch.no_grad():
            ...
            V_pred = V_net(U_grid_tensor.unsqueeze(1))
            V_pred_np = V_pred.cpu().numpy().flatten()
            V_predS.append(V_pred_np)
    ...
    np.save(f'{save_dir}/V_predS.npy', np.array(V_predS))
    ...
    model_performance_eval(save_dir, u_net, D_net, test_loader, device, t_train, x_train, U_grid_tensor, Nt, u_true, \
               D, U_grid, u_min, u_max, X_grid, T_grid, BCloss_S, pdeloss_S, val_BCloss_S, val_pdeloss_S, Numrep, store_loss_step, \
                    val_u_bio_S, val_D_bio_S, u_bio_S, D_bio_S, uloss_S, val_uloss_S, Num_epochs, V_true, V_net, bio_constraint_bool)
    pass
